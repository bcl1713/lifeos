...
- Wiki checkbox discovery: `docs/wiki-checkbox-task-discovery.md` documents the reviewed-but-not-yet-merged scanner-authoritative Today/Tasks and API contract. Checkbox labels can carry ordered supporting Markdown/wiki context links; typed metadata requires the exact lowercase `task:` destination marker, with one-time payload decoding (`%20` becomes a path space). The guide also covers diagnostics, safe source navigation, refresh/watcher recovery, and the boundary against source writes, SQL fallback, fetches, real-wiki apply, deployment, and `main` action.
