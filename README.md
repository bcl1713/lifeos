...
- Wiki checkbox discovery: `docs/wiki-checkbox-task-discovery.md` documents the reviewed-but-not-yet-merged scanner-authoritative Today/Tasks and API contract, safe source navigation, refresh/watcher recovery, and the boundary against source writes, SQL fallback, real-wiki apply, deployment, and `main` action.
